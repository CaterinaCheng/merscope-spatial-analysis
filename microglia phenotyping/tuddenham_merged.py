"""
341_tuddenham_merged.py
Same as 340 but with MERGED meta-groups (12 fine Tuddenham clusters collapse into
a smaller, biologically coherent set). Per user: combine homeostatic clusters
2/3/4; also pool the redundant pairs (1/6 Metabolic, 10/13 APC). Signatures are
built by POOLING each group's up-genes (sum_logFC ranked, deduped, panel+BLOCK
filtered), then scoring decontam microglia. UMAP + compartment/T-subset enrichment.
"""
import sys
...
VESSEL=["End","Per","SMC"]; TOPN=18; MINGENES=6
# Tuddenham fine cluster -> MERGED meta-group (homeo 2/3/4 combined; 1/6 metabolic; 10/13 APC)
GROUP={2:"Homeostatic",3:"Homeostatic",4:"Homeostatic",
       1:"Metabolic/Tx",6:"Metabolic/Tx",
       7:"Stress",
       9:"APOE/Lipid",
       5:"Motility",
       8:"Cytokine/IL",
       10:"APC (HLA/Compl)",13:"APC (HLA/Compl)",
       11:"DAM/GPNMB",
       12:"Proliferative"}
GORDER=["Homeostatic","Metabolic/Tx","Stress","APOE/Lipid","Motility","Cytokine/IL","APC (HLA/Compl)","DAM/GPNMB","Proliferative"]
...
vset=set(var)
# build pooled signature per merged group
SIG={}
for grp in GORDER:
    cls=[cl for cl,gg in GROUP.items() if gg==grp]
    sub=up[up.up_type.isin(cls)].sort_values("sum_logFC",ascending=False)
    genes=[]; seen=set()
    for gn in sub.gene:
        if gn in vset and gn not in BLOCK and gn not in seen:
            genes.append(gn); seen.add(gn)
        if len(genes)>=TOPN: break
    if len(genes)>=MINGENES: SIG[grp]=genes
print(f"merged meta-groups scoreable (>= {MINGENES} panel markers): {len(SIG)} of {len(GORDER)}")
for k,v in SIG.items(): print(f"  {k:18}: {', '.join(v)}")
drop=[g for g in GORDER if g not in SIG]; print("dropped (too few panel markers):",drop)
...
print("\ndominant meta-group composition:",{k:round(100*(A.obs.subtype==k).mean(),1) for k in STATEORD+['Mixed/low'] if (A.obs.subtype==k).any()})
A.obs[STATEORD+["subtype"]].to_csv(NEW/"microglia_tuddenham_merged_labels.csv")
# spatial
...
RC.to_csv(NEW/"tuddenham_merged_compartment.csv",index=False); RT.to_csv(NEW/"tuddenham_merged_Tsubset.csv",index=False)
print("\ncompartment enrichment (Cliffs delta vs parenchymal, *=padj<0.05):")
for st in STATEORD:
    row=RC[RC.factor==st]
    s=" | ".join(f"{r.grp[:4]} {r.cliffs:+.2f}{'*' if r.padj<0.05 else ''}" for _,r in row.iterrows())
    print(f"  {st:18}: {s}")
# UMAP
co=pd.read_csv(NEW/"microglia_umap_coords.csv",index_col=0); U=co.reindex(A.obs_names)[["umap1","umap2"]].values
order=STATEORD+["Mixed/low"]; pal=cm.get_cmap("tab10",max(len(order),3))
...
ax1.set_title("Microglia UMAP — Tuddenham2024 merged meta-groups (decontam)",fontsize=10.5,fontweight="bold")
...
plt.tight_layout(); fig.savefig(NEW/"microglia_umap_tuddenham_merged.png",dpi=150,bbox_inches="tight"); plt.close()
print("\nSaved: microglia_umap_tuddenham_merged.png + tuddenham_merged_{compartment,Tsubset}.csv + microglia_tuddenham_merged_labels.csv")
